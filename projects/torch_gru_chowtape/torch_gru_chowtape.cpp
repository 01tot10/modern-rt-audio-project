...
{
    while((--path.end())->string() != "modern-rt-audio-project")
...
std::string getModelFile(fs::path path)
{
    ...
    path.append("weights/gru_torch_chowtape.json");
...
{
    path = getRootDir(path);
    path.append("test_data/gru_torch_chowtape_x_python.csv");
    return path.string();
}
...
{
    path = getRootDir(path);
    path.append("test_data/gru_torch_chowtape_y_python.csv");
    return path.string();
}
...
int main([[maybe_unused]] int argc, char* argv[])
{
    ...
    {
        std::cout << "Expected: " << expected_y[i] << std::endl;
        std::cout << "Produced: " << outputs[i] << std::endl;
    }
